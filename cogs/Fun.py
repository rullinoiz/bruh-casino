...
class Fun(BruhCasinoCog):
    ...
    async def snipe(self, ctx: Interaction) -> None:
        sniped: discord.Message = self.get_sniped(ctx.guild.id, ctx)

        if sniped:
            embed = discord.Embed(
                description=sniped.content
            ).set_author(name=sniped.author.name, icon_url=sniped.author.display_avatar.url).set_footer(text=bcfg["footer"])
        else:
            raise SnipeFailed()

        await ctx.response.send_message(embed=embed, delete_after=None if sniped else 7)
