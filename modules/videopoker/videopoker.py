...
class HandAnalyzer(object):
    ...
    def pay_current_hand(self, allpays: int = 0, bet: int = 0):
        """
        ...
        names = {
            'pair_jqka': 'PAIR OF JACKS OR BETTER',
            'two_pair': 'TWO PAIR',
            ...
            'four_kind7': 'SEVEN SEVEN SEVEN SEVEN',
            'four_kindA8': 'CRAZY ACES'
        }
        result: str = ''
        if allpays == 0: return pays
        if allpays == 1: return max(pays.values())
        if allpays == 3:
            if (m := max(pays.values())) == 0:
                return ''
            result += names[list(filter(lambda x: pays[x] == m, pays))[0]] + ' '
        if allpays in [2, 3]:
            result += f'(${bet * max(pays.values())})'
    ...
    def two_pair(self):
        ...
        # most bc_common card is a singleton
            ...
            if self.draw_cnt == 4:
                ...
                # possibly replace this block with a call to draw_for_ranks?
