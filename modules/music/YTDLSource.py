    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: typing.Union[asyncio.BaseEventLoop, asyncio.AbstractEventLoop] = None) -> typing.Union[T, list[T]]:
        ...
        if '_type' in data.keys() and data['_type'] == 'playlist' and not data['webpage_url'].startswith('ytsearch'):
            if data['playlist_count'] > 20:
                raise exceptions.TooMuchData(
                    f'Maximum playlist item count is 20! (received {data["playlist_count"]} songs)')

            sources = [cls(ctx, discord.FFmpegPCMAudio(x['url'], **cls.FFMPEG_OPTIONS), data=x) for x in
                       data['entries']]
            return sources
